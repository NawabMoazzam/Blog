...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.header
      ...
      className={cn("sticky inset-x-0 top-0 z-40 w-full h-0", className)}
    >
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.nav
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-muted-foreground transition duration-200 hover:text-muted lg:flex lg:space-x-2",
        className
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarButton = <T extends React.ElementType = "a">({
  ...
  return React.createElement(Tag, componentProps as any, children);
};
