...
// Pre-generate arcs data outside component to avoid recalculation
function generateSampleArcs(colors: string[]) {
  const arcs = [
...
function GlobeRenderer() {
  ...
  // Memoize arcs data - only regenerate if colors change
  const sampleArcs = useMemo(() => generateSampleArcs(GLOBE_COLORS), []);

  // Memoize globe config to prevent unnecessary recalculations
  const globeConfig = useMemo(() => ({
    pointSize: 4,
    globeColor: currentTheme === "dark" ? "#062056" : "#0c40ab",
    showAtmosphere: true,
    atmosphereColor: currentTheme === "dark" ? "#FFFFFF" : "#38bdf8",
    atmosphereAltitude: 0.1,
    emissive: currentTheme === "dark" ? "#062056" : "#0c40ab",
